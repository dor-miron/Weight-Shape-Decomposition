...
from utils.other_utils import sigfiground, calibrate_energies, get_random_string
...
with times_agg('Get data'), st.spinner('Get data'):
    ...
    max_n = st.sidebar.number_input('Max N', min_value=0, value=6, step=1)
    dataset.filter_by_number_of_particles(max_n=max_n)
...
def dataset_exploration():
    ...
    def main():
        ...
        def show_islands():
            ...
            cluster_thresh = st.sidebar.number_input('Cluster Threshold', min_value=0.0, value=0.0, step=0.01)
            ...
            for ind, event_id in enumerate(dataset.keys()):
                ...
                pred_n_array[ind] = island_estimator.cluster_count_
                pred_n_dict[event_id] = island_estimator.cluster_count_
            ...
            def show_statistics():
                ...
                fig = px.histogram(x=all_clusters, title="Histogram of all clusters")
                fig.update_layout(get_plotly_layout(xaxis_title='Cluster Energies (MeV)'))
                my_plotly_chart(histogram_cols[0], fig)

                small_clusters = all_clusters[all_clusters < 5]
                fig = px.histogram(x=small_clusters, title="Histogram of small clusters")
                fig.update_layout(get_plotly_layout(xaxis_title='Cluster Energies (MeV)'))
                my_plotly_chart(histogram_cols[1], fig)
            ...
            for event_id in true_energies_dict:
                cur_pred_energies = estimators[event_id].sum_per_cluster
                pred_energies_dict[event_id] = cur_pred_energies

            if st.sidebar.checkbox('Filter by Pred energy', value=False):
                ...
                max_pred_energy_threshold = st.sidebar.number_input(
                    label='Pred energy max threshold', min_value=0.0, max_value=max_energy, value=max_energy,
                    step=0.001,
                )
                min_pred_energy_threshold = st.sidebar.number_input(
                    label='Pred energy min threshold', min_value=0.0, max_value=max_energy, value=0.0,
                    step=0.001,
                )
            else:
                min_pred_energy_threshold, max_pred_energy_threshold = 0, np.inf
            ...
            do_calibration = st.sidebar.checkbox(
                'Do Calibration', value=False, disabled=not is_show_islands, key='ISLANDS CALIBRATION CHECKBOX')
            if do_calibration:
...
def event_exploration():
    ...
    with times_agg('Islands clustering'):
        ...
        if is_show_1d_tbc:
            ...
            plotly_df = pd.DataFrame({'Energy Deposition': island_estimator.raw_1d_, 'Cluster Number': island_estimator.ind2cluster_})
            fig = px.bar(plotly_df, x=plotly_df.index, y='Energy Deposition', color='Cluster Number')
            fig.update_layout(get_plotly_layout(1, 1))
            st_cols[0].plotly_chart(fig)

            sorted_energies = sigfiground(island_estimator.get_sum_per_cluster(), 3)
            fig = go.Figure()
