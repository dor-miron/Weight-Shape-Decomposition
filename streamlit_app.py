...
from pipelines.estimators import get_sum_per_island, IslandEstimator
...
with times_agg('Get data'), st.spinner('Get data'):
    ...
    calo, energies = dataset.calo_dict, dataset.energies_dict
...
def dataset_exploration():
    ...
    def main():
        ...
        with times_agg('get_all'):
            events_array = get_events_array(stride)
        ...
        with times_agg('Islands predictor'):
            ...
            is_show_islands = st.sidebar.checkbox('Show Islands predictor', value=True)
            ...
            if is_show_islands:
                ...
                for ind, event in enumerate(dataset.calo_dict.values()):
                    progress_placeholder.header(f'Islands predictor -  {ind + 1}/{n_events}')
                    _, cluster_count = island_estimator.predict_one(event)
                ...
                correct_mask = true_n_array == pred_n_array
...
def event_exploration(dataset_key):
    ...
    with times_agg('Islands clustering'):
        ...
        if is_show_1d_tbc:
            island_estimator = IslandEstimator()
            ind2cluster, _, data_to_fit = island_estimator.predict_one(calo_event, return_data=True)
